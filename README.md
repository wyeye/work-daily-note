...
- 设置页分为基础设置、应用更新、分类管理和高级设置，API Key 默认隐藏显示。
- 一键复制日报文本，手动粘贴到目标系统。
- 设置页可手动“检查更新”，从 GitHub Releases 读取 `latest.json` 并安装新版。
- 设置页支持分类管理，分类会保存到 `sync-data/settings/categories.json` 并同步到记录页。
...
5. 设置页默认显示 AI 地址、API Key、模型名、提醒时间、应用更新和分类管理；点击“检查更新”可检测并安装 GitHub Releases 中的新版本。
6. 分类管理中新增或删除分类后保存设置，记录页下方的分类标签会同步更新。
7. 高级设置收起保存日报模板、项目识别规则、启动页、Enter 行为和提醒策略。
